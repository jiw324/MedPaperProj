...
# **Intent:** Provide a lightweight CLI/REPL to run ClinicalBERT for masked LM or embeddings.
...
"""
ClinicalBERT Text CLI
=====================

Supported tasks:
- fill-mask (default): predict tokens for `[MASK]`
- embeddings: return pooled sentence embeddings (prints vector length/shape, not full vector)

Usage (PowerShell):
  cd "...\Medical LLM Model\ClinicalBERT"
  $env:HF_TOKEN="hf_..."   # if gated/private; do NOT paste tokens into chat
...
def _load_pipeline(task: str, model_id: str, hf_token: Optional[str], device_map: str):
    ...
    if task == "fill-mask":
        ...
        return pipeline("fill-mask", model=model, tokenizer=tok, device_map=device_map)
    if task == "embeddings":
        model = AutoModel.from_pretrained(model_id, device_map=device_map, token=hf_token)
        return pipeline("feature-extraction", model=model, tokenizer=tok, device_map=device_map)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="ClinicalBERT CLI for fill-mask or embeddings.",
