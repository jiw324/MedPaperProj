"""
llama33_groq_runner.py
// AI-Generated Code Header
// **Intent:** Run Llama 3.3 remotely on Groq using prompts.jsonl (JSON message arrays) and write generations JSONL.
...
DEFAULT_MODEL = (
    os.environ.get("LLAMA33_GROQ_MODEL")
    or os.environ.get("GROQ_MODEL")
    or "llama-3.3-70b-versatile"  # AI-SUGGESTION: common Groq model id; override if needed.
).strip()
...
def _apply_attack_override(*, messages: List[Dict[str, str]], row: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    if not isinstance(mapping, dict):
        return messages
    attack_msg = mapping.get("llama-3.3") or mapping.get("default")
    ...
    *,
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if verbose:
                print(f"[llama33-groq] retry={attempt+1}/{max_retries} sleep={sleep_s:.2f}s err={e}", file=sys.stderr)
            time.sleep(sleep_s)
    raise RuntimeError(f"Groq request failed after {max_retries + 1} attempts: {last_err}") from last_err


def main(argv: Optional[Sequence[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Run Llama 3.3 on Groq using prompts.jsonl and write generations JSONL.")
    ap.add_argument("--model", default=DEFAULT_MODEL, help="Groq model name for Llama 3.3.")
    ...
    ap.add_argument(
        "--out_jsonl",
        type=Path,
        default=Path("Research_setup/output/allergy_omission/results/generations_llama33_groq.jsonl"),
    )
    ...
    for row in iter_jsonl(args.prompts_jsonl):
        ...
        try:
            ...
            if args.print_messages or args.verbose:
                print(f"[llama33-groq] case={case_id} messages={json.dumps(msgs, ensure_ascii=False)}", file=sys.stderr)
