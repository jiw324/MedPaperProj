...
// **Intent:** Provide a minimal, robust API wrapper + CLI to run GPT-4.1 on JSON chat prompts and save generations.
...
def call_gpt41_chat(
    *,
    ...
    """
    Call GPT-4.1 using the OpenAI Chat Completions API via the official SDK.
...
def run_prompts_jsonl(
    *,
    ...
    for row in iter_jsonl(prompts_jsonl):
        ...
        n += 1
        if n % 25 == 0:
            print(f"[gpt41] processed {n} prompts", file=sys.stderr)

    write_jsonl(out_jsonl, rows_out)


def main(argv: Optional[Sequence[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Run GPT-4.1 on prompts.jsonl produced by allergy_omission_experiment.py")
    ap.add_argument(
        "--prompts_jsonl",
        type=Path,
    ...
    ap.add_argument(
        "--out_jsonl",
        type=Path,
        default=Path("Research_setup/output/allergy_omission/results/generations_gpt41.jsonl"),
    )
