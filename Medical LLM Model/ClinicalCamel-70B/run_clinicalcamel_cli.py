...
# **Intent:** Provide a text-based interface (CLI/REPL) to run Clinical Camel via Transformers.
...
Notes:
- This is a 70B model and typically requires multi-GPU or quantization.
- If the repo is gated, set HF_TOKEN before running.
...
def _fallback_prompt(messages: List[Dict[str, str]]) -> str:
    # Simple fallback if chat template isn't available.
    parts: List[str] = []
    for m in messages:
...
def _build_prompt(tokenizer, messages: List[Dict[str, str]]) -> str:
    # AI-SUGGESTION: Prefer model-provided chat_template if present; fall back otherwise.
    try:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Clinical Camel (Transformers) text interface.",
    ...
    parser.add_argument(
        "--system",
        default="You are a helpful clinical language model. Provide educational information only and encourage consulting clinicians for medical decisions.",
    )
