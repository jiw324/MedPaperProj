...
# **Intent:** Evaluate PubMedGPT 2.7B (BioMedLM) on small subsets of medical QA datasets (PubMedQA/MedMCQA).
# **Optimization:** Subset-based evaluation; supports `device_map=auto` and optional quantization for feasibility.
# **Safety:** Clear error messages for gated access and OOM; avoids huge downloads by default.

"""
PubMedGPT 2.7B (BioMedLM) Experiment: Medical QA (subset)
=========================================================
...
def build_prompt(ex: Example) -> str:
    # BioMedLM is not necessarily chat-tuned; keep prompts concise.
    if ex.choices:
        opts = "\n".join([f"{chr(ord('A') + i)}. {c}" for i, c in enumerate(ex.choices)])
        return (
            "Answer the medical question by choosing the best option.\n\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Subset evaluation for PubMedGPT 2.7B (BioMedLM).",
